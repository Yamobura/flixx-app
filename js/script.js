...
<div class="details-bottom">
  <h2>Movie Info</h2>
  <ul>
    <li><span class="text-secondary">Budget:</span> $${new Intl.NumberFormat('en-US').format(Number(movie.budget))
    }</li>
    <li><span class="text-secondary">Revenue:</span> $${
        new Intl.NumberFormat('en-US').format(Number(movie.revenue))
    }</li>
...
async function fetchAPIData(endpoint) {
    try {
        const APIDetails = await getAPIDetails();

        const API_KEY = APIDetails.API_KEY;
        const API_URL = APIDetails.API_URL;
...
async function getAPIDetails() {
    try {
        ...
        const apiKeyText = await response.text();
        const apiKey = apiKeyText.split("'")[1];
